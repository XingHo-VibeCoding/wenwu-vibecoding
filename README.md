...
单文件静态站：HTML + 内联 CSS + 一段内联 JS（本地 mock 数据渲染主视图），**零后端、零网络请求、零构建、零依赖**——打开 `index.html` 就能用。
...
- **主视图（mock 演示）**：打开页面先见加载骨架屏，随后渲染 4 张场景卡 + 6 张资源卡；可用"状态演示"按钮或 `?state=empty` / `?state=error` 查看空态与错误态（错误态带重试）
- **按任务跳转**：顶部一排任务按钮（拍板书、交作业、传安装包、同步笔记等 8 个），点击即跳对应场景锚点——纯 HTML 表单实现
