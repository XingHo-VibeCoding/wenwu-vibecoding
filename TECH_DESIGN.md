...
```
wenwu-vibecoding/
├── index.html        # 整站唯一页面：页头（任务跳转按钮+速查）→ 主视图（mock 渲染，Day 8）→ S1–S4 场景板块 → 资源入口 → 页脚
...
访问者浏览器（加载 index.html，一次性拿到全部内容）
        │  交互：mock 数据渲染主视图（Day 8）/ 任务按钮跳转 / 锚点跳转（均由本地能力完成）
        ▼
...
4. **自查顺序**：AC-01（本地打开）→ AC-02（无外部 JS 引用 / 零网络请求）→ AC-03（断网）→ AC-04~06（跳转按钮与锚点）→ AC-07~09（场景结构）→ AC-10（资源链接）→ AC-11（无越界功能）→ AC-12（单 HTML 文件）→ AC-13（四状态，Day 8）。
